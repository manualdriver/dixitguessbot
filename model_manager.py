...
class ModelManager:
    ...
    def initialize_model(self):
        with self._lock:
            if self.model is None or self.transform is None:
                print(f"Loading model {self.model_name} with pretrained weights: {self.pretrained}")
                try:
                    ...
                    print(f"Model loaded in {load_duration:.2f} seconds on device: {self.device}.")
                    self.model_loading_complete = True
                except Exception as e:
                    print(f"Error initializing model: {e}", exc_info=True)
                    raise
    ...
    def apply_quantization(self):
        ...
        try:
            ...
            print(f"Error during model quantization: {e}", exc_info=True)
            raise
    ...
    def load_model_weights(self, weights_path):
        ...
        try:
            ...
            print(f"Error loading model weights: {e}", exc_info=True)
            raise
    ...
    def save_model_weights(self, save_path):
        ...
        try:
            ...
            print(f"Error saving model weights: {e}", exc_info=True)
            raise
    ...
    def initialize_model_async(self, callback=None):
        ...
        def load_model():
            try:
                ...
                print(f"Error during async model initialization: {e}", exc_info=True)
                if callback:
                    callback(e)
    ...
    def log_gpu_memory(self):
        ...
        if torch.cuda.is_available():
            ...
            print("GPU is not available; skipping GPU memory logging.")

    def profile_model(self, input_tensor):
        """Profile the model's performance using a given input tensor."""
        if not torch.cuda.is_available():
            print("Profiling on CPU as GPU is not available.")
